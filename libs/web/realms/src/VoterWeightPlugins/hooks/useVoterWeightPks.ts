...
export const useVoterWeightPks = (
  ...
  | { voterWeightPks: undefined; maxVoterWeightPk: undefined }
  | { voterWeightPks: PublicKey[]; maxVoterWeightPk: PublicKey | undefined },
  ...
  useAsync(async () => {
    ...
    if (plugins.length === 0) {
      const tokenOwnerRecords = walletPublicKeys.map((walletPublicKey) =>
        getTokenOwnerRecordAddressSync(realmPublicKey, governanceMintPublicKey, walletPublicKey),
      )
      return {
    ...
    const voterWeightPks = await Promise.all(
      walletPublicKeys.map((walletPublicKey) =>
        lastPlugin.client
          .getVoterWeightRecordPDA(realmPublicKey, governanceMintPublicKey, walletPublicKey)
          .then((pda) => pda.voterWeightPk),
      ),
    )
    const { maxVoterWeightPk } =
      (await lastPlugin.client.getMaxVoterWeightRecordPDA(realmPublicKey, governanceMintPublicKey)) || {}

    return {
      voterWeightPks,
      maxVoterWeightPk,
    }
  }, [args.plugins?.length, args.walletPublicKeys?.map((pk) => pk.toBase58()).join(',')])
