...
import { findPluginName, PluginName } from '@realms/constants/plugins'
import { loadClient } from '../clients/'
import { BN, Provider, Wallet } from '@coral-xyz/anchor'
import { getRegistrarPDA as getPluginRegistrarPDA } from '@realms/utils/plugin/accounts'
import { PluginType, VoterWeightPluginInfo } from './types'
import { fetchRealmByPubkey } from '@realms/hooks/queries/realm'
...
const getInitialPluginProgramId = async (
  ...
  const governanceConfig =
    config.result?.account?.[kind === 'community' ? 'communityTokenConfig' : 'councilTokenConfig']
...
const weightForWallet = async (
  ...
  if (type === 'voterWeight') {
    const voterWeightRecord = (await client.getVoterWeightRecord(realmPublicKey, governanceMintPublicKey, wallet)) as {
      voterWeight: BN
    } | null
    ...
    const maxVoterWeightRecord = (await client.getMaxVoterWeightRecord(realmPublicKey, governanceMintPublicKey)) as {
      maxVoterWeight: BN
    } | null
...
export const getPlugins = async ({
  ...
  if (programId) {
    ...
    do {
      ...
      const client = await loadClient(pluginName as PluginName, programId, provider, signer)
      ...
      const registrarData = await client.getRegistrarAccount(realmPublicKey, governanceMintPublicKey)
