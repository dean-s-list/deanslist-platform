...
export const useDelegators = (role: 'community' | 'council' | undefined) => {
  ...
  const relevantMint = role && role === 'community' ? realm?.account.communityMint : realm?.account.config.councilMint

  const { data: torsDelegatedToUser } = useTokenOwnerRecordsDelegatedToUser()
  const relevantDelegators =
    relevantMint && torsDelegatedToUser?.filter((x) => x.account.governingTokenMint.equals(relevantMint))
