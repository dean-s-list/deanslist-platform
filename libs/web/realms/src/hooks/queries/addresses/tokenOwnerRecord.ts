...
export const useAddressQuery_CouncilTokenOwner = () => {
  ...
  const owner = selectedCouncilDelegator !== undefined ? selectedCouncilDelegator : wallet?.publicKey ?? undefined

  return useAddressQuery_TokenOwnerRecord(realm?.owner, realm?.pubkey, realm?.account.config.councilMint, owner)
}
...
export const useAddressQuery_CommunityTokenOwner = () => {
  ...
  return useAddressQuery_TokenOwnerRecord(realm?.owner, realm?.pubkey, realm?.account.communityMint, owner)
}
...
export const useAddressQuery_TokenOwnerRecord = (
  ...
  const enabled =
    owner !== undefined && governingTokenMint !== undefined && realmPk !== undefined && programId !== undefined

  return useQuery({
    queryKey: enabled ? Object.freeze(['TokenOwnerAddress', [programId, realmPk, governingTokenMint, owner]]) : [],
    queryFn: async () => {
      if (!enabled) throw new Error()

      return getTokenOwnerRecordAddress(programId, realmPk, governingTokenMint, owner)
    },
    enabled,
