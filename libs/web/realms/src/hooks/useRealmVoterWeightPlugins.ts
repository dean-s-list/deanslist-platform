...
import {
  useVoterWeightPlugins,
  UseVoterWeightPluginsReturnType,
} from '@realms/VoterWeightPlugins/useVoterWeightPlugins'
import { useRealmQuery } from '@realms/hooks/queries/realm'
import useWalletOnePointOh from '@realms/hooks/useWalletOnePointOh'
import { GovernanceRole } from '@realms/@types/types'
import { useSelectedDelegatorStore } from '@realms/stores/useSelectedDelegatorStore'
import { PublicKey } from '@solana/web3.js'
import { CalculatedWeight } from '@realms/VoterWeightPlugins/lib/types'
import { useDelegators } from '@realms/components/VotePanel/useDelegators'
...
export const useRealmVoterWeightPlugins = (
  ...
  const walletPublicKeys = getWalletList(
    selectedDelegator,
    delegators?.map((programAccount: any) => programAccount.account),
    wallet,
  )
  ...
  const totalCalculatedVoterWeight = nonAggregatedResult.calculatedVoterWeights?.length
    ? nonAggregatedResult.calculatedVoterWeights?.reduce((acc, weight) => {
        ...
        const value =
          weight.value === null ? (acc.value === null ? null : acc.value) : weight.value.add(acc.value ?? BN_ZERO)
...
//   const { calculatedMaxVoterWeight: communityMaxWeight, totalCalculatedVoterWeight: communityWeight } =
//     useRealmVoterWeightPlugins('community')
//   const { calculatedMaxVoterWeight: councilMaxWeight, totalCalculatedVoterWeight: councilWeight } =
