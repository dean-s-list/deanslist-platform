...
export function getFormattedStringFromDays(numberOfDays: number, fullFormat = false) {
  ...
  const yearsDisplay = years > 0 ? years + `${fullFormat ? yearSuffix : 'y'}` : null
  const monthsDisplay = months > 0 ? months + `${fullFormat ? monthSuffix : 'm'}` : null
  const daysDisplay = days > 0 ? days + `${fullFormat ? daysSuffix : 'd'}` : null
...
export const getTimeLeftFromNowFmt = (ts: BN) => {
  ...
  return getFormattedStringFromDays(ts.sub(dateNowSecTimeStampBN).toNumber() / SECS_PER_DAY)
}
